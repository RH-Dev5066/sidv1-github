...
async function initGaleriFilter() {
    ...
    const facebookFeedUrl = 'https://api.rss2json.com/v1/api.json?rss_url=https://t.ly/bontobontoa';
    const targetKeywords = ['sosialisasi', 'ramadhan', 'tausiah', 'kerja bakti', 'jumat bersih', 'seminar', 'rapat', 'bpd', 'musyawarah', 'kerja'];
    
    let allGalleryData = [...manualImages];

    try {
        if (!facebookFeedUrl.includes('CONTOH')) {
            ...
            if (result.status === 'ok' && result.items) {
                ...
                result.items.forEach(item => {
                    ...
                    const hasKeyword = targetKeywords.some(keyword => content.includes(keyword) || title.includes(keyword));
                    ...
                    if (hasKeyword) {
                        ...
                        if (imageUrl) {
                            ...
                            let category = 'kegiatan';
                            if (content.includes('infrastruktur')) category = 'infrastruktur';
                            if (content.includes('budaya')) category = 'budaya';

                            const tempDiv = document.createElement('div');
                            tempDiv.innerHTML = item.title || item.description;
                            const cleanCaption = tempDiv.textContent.split(" ").slice(0, 9).join(" ") + "...";

                            allGalleryData.push({
                                url: imageUrl, category: category, caption: cleanCaption, date: postDate
                            });
                        }
                    }
                });
        ...
        console.warn("Gagal mengambil data Facebook:", error);
    }
    ...
    allGalleryData.forEach((item, index) => {
        ...
        galeriItem.innerHTML = `
            <img src="${finalUrl}" alt="${item.caption}" loading="lazy" 
                 onerror="this.onerror=null; this.src='https://via.placeholder.com/400x300?text=Gagal+Load';">
...
function processImageUrl(url) {
    ...
    if (url.includes('fbcdn.net') || url.includes('facebook.com') || url.includes('cdninstagram')) return `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=1920&q=90&output=webp`;
    return url;
}
...
function initFileUpload() {
    ...
    if (!wrapper || !input || !preview || !removeBtn) return;

    ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => wrapper.addEventListener(eventName, preventDefaults, false));
    function preventDefaults(e) { e.preventDefault(); e.stopPropagation(); }
    ['dragenter', 'dragover'].forEach(eventName => wrapper.addEventListener(eventName, () => wrapper.classList.add('dragover'), false));
    ['dragleave', 'drop'].forEach(eventName => wrapper.addEventListener(eventName, () => wrapper.classList.remove('dragover'), false));
    ...
    input.addEventListener('change', function() { if (this.files && this.files[0]) updateFilePreview(this.files[0]); });
    removeBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); input.value = ''; wrapper.classList.remove('has-file'); preview.classList.remove('active'); });

    function updateFilePreview(file) {
        if (file.size > 10 * 1024 * 1024) { showNotification('error', 'File Terlalu Besar', 'Ukuran file maksimal 10MB.'); input.value = ''; return; }
        if (!['image/jpeg', 'image/png', 'image/jpg', 'application/pdf'].includes(file.type)) { showNotification('error', 'Format Tidak Didukung', 'Gunakan format JPG, PNG, atau PDF.'); input.value = ''; return; }
...
function showConfirmation(form) {
    ...
    const fileInput = form.querySelector('#lampiran');
    ...
    summaryContainer.innerHTML = `
        ...
        <div class="confirmation-summary-item"><span>Pesan</span><span>${escapeHtml(pesan.length > 80 ? pesan.substring(0, 80) + '...' : pesan)}</span></div>
        <div class="confirmation-summary-item"><span>Lampiran</span><span>${hasFile ? escapeHtml(fileInput.files[0].name) : 'Tidak ada'}</span></div>
    `;
...
function validateEmailInput(email) {
    ...
    // Validasi Karakter Domain
    const validDomainChars = /^[a-z0-9.-]+$/;
    if (!validDomainChars.test(domain)) { showEmailError('Domain mengandung karakter tidak valid. Hanya huruf, angka, titik (.), dan strip (-) yang diperbolehkan.'); return false; }
    if (domain.startsWith('.') || domain.endsWith('.') || domain.startsWith('-') || domain.endsWith('-')) { showEmailError('Domain tidak boleh dimulai atau diakhiri dengan titik (.) atau strip (-).'); return false; }
    if (domain.includes('..') || domain.includes('--')) { showEmailError('Domain tidak boleh memiliki titik (.) atau strip (-) ganda yang berurutan.'); return false; }

    // Validasi Jenis Domain (Whitelist)
    if (EMAIL_CONFIG.blockedDomains.includes(domain)) { showEmailError('Email sementara/sekali pakai tidak diperbolehkan. Gunakan email permanen Anda.'); return false; }
    ...
    if (!isAllowedFreeDomain && !isSchoolDomain) {
        showEmailError('Email tidak valid. Gunakan email Gmail, Yahoo, Outlook, atau email institusi pendidikan (.sch.id / .ac.id).');
        return false;
    }
    
    if (email.length > 254) { showEmailError('Email terlalu panjang. Maksimal 254 karakter.'); return false; }
...
function showEmailSuccess(isSchool) {
    ...
    if (emailFeedback) {
        const message = isSchool ? `<i class="bi bi-check-circle-fill"></i>Email institusi pendidikan valid.` : `<i class="bi bi-check-circle-fill"></i>Email valid dan siap digunakan.`;
...
function initFormValidation() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        inputs.forEach(input => {
            ...
            if (!input.value.trim()) {
                isValid = false; if (!firstInvalid) firstInvalid = input; input.style.borderColor = '#ef4444';
            }
        });
        
        const emailInput = form.querySelector('#email');
        if (emailInput && emailInput.value.trim()) {
            if (!validateEmailInput(emailInput.value.trim())) { isValid = false; if (!firstInvalid) firstInvalid = emailInput; }
        }
...
function checkFormStatus() {
    ...
    if (urlParams.has('sent')) {
        showNotification('success', 'Pesan Terkirim!', 'Terima kasih telah menghubungi kami. Tim kami akan segera merespons pesan Anda.');
